...
独立skill_diagnostic运行器只使用已冻结23条dev输入，固定输出reports/diagnostics/v02_skill_projection_v0.1；不更改旧evaluate默认协议。spec和packet SHA固定，核验来源/Adapter实际SHA/全配置，逐题原子保存、OS锁、失败记录与幂等续跑。任何截断保留原始证据后停止，不悄悄延长生成或重试。五项模拟中断/输入篡改/非dev/指纹/截断测试通过；不是模型诊断结果。

依用户新要求，所有23条均逐题审查并比较动作与语义。A要求角色理解、合法动作和技能幻觉共同明确改善且无大规模语义退化；若证据混合、不足以支持A，保守进入B，优先日程诊断；整体明确变差进C。诊断为dev探索，不以统计显著性或held-out提升包装。分支尚未选择，不在观察后修改23条题目或原评分。

## 投影诊断规则dev检查点

7条规则dev复核完成，动作和语义分别记录；投影未普遍消除技能规则幻觉，不能据none→pass宣布根因。源回答SHA与23题全量报告框架已保存。后续继续同一运行，不改参数/输入，也不挑选局部收益。

## 投影诊断策略组检查点

12条dev已完成SHA绑定模型复核；动作类型合法化与策略改进不等价。所有改善、混合、无变化和退步均保留，分支仍待全量。

## 投影诊断反事实检查点

18条模型复核完成，三对双正确均0/3。明确区分动作发生变化与合理条件更新。猎人公开技能宣告不自动等同遗言违规，缺少明确边界证据时记null，不夸大泄漏统计。继续5条Blind后正式判分支。

## 投影诊断完成并进入分支B（2026-09-15）

23/23 dev生成和逐题模型复核完成；恢复时原进程已结束且OS锁空闲，不重复生成。合法3→7、严格命中1→2、未知词14→10，三对双正确仍0/3；明确承认局部收益。技能幻觉持续，队伍确定知识有退步，夜间泄漏未减少，不能将其包装为A的核心干扰因素已证实。按执行前规定，混合且不满足A的结果归B，保留旧Schema主线，优先设计训练日程诊断，尚不宣称日程就是主因。

全部23条及改善/混合/无变化/退步均保留。模型审核非独立人审，role_skill包含错误角色/技能，状态维度包含历史/task使用，维度可重叠不相加。公开技能宣告与遗言分开，缺证据记null。Blind-04公开资料含目标12提示，不能以它单独宣称完全自主；不改旧题或追溯评分。

正式报告包含已证实/有支持证据/暂不支持/尚未验证；下一阶段先设计A/B/C/D最小日程矩阵和配置，不换Base、数据、模板/标签/dev，不并入视频/Persona，不开Phase2/RL/竞技场，不重写训练框架。

## Training Schedule Diagnostic矩阵冻结（2026-09-15）

A复用已完成v0.2，不重训。B仅改warmup_ratio=0；C以B为对照仅改累积16→8；D以B为对照仅改epoch1→2。输出目录隔离，不是模型变量。Base、数据/家族划分/messages/Chat Template/标签/23条旧dev输入/评分/推理参数完全固定。下一次只推进B，C/D按前一诊断结果条件执行，不做网格搜索。

CPU余弦scheduler核验给出A/B/C/D总步6/6/10/12，非零LR3/6/10/12。A与现有日志吻合；B每阶段计划[5e-5,2.5e-5]，取消预热也改变LR曲线/累计更新强度，不能只归因于次数。只有A是真实训练证据，B/C/D所有实际权重/日志/新dev成绩尚不存在。

保留阶段best/final选择规则以避免同时改变选择政策；报告必须区分执行总更新数与最终所选checkpoint累计更新数。C改变有效批量和频率、D改变曝光量，不能直接用C/D差异宣称单一因果。阶段间遗忘尚未证实，优先以已有Rule/Strategy Adapter做同dev阶段面板后再考虑训练组织改变。

离线预检验证配置差异白名单、固定23条控制输入、真实Tokenizer/原manifest与本地Trainer日程；6个快照二次运行SHA/mtime不变，B/C/D输出目录不存在。恢复命令已指向B独立配置，旧冻结数据、权重、预测不改。

## 2026-09-15 B正式启动门禁

用户授权按冻结矩阵执行B三阶段；已通过GPU NF4前反向、三阶段dry-run和原dev输入核验。唯一训练变量warmup=0，每阶段实际LR预计[5e-5,2.5e-5]，同时改变cosine总LR积分，不能把实验表述为仅多一个等幅更新。保持原按阶段valLoss选best/final的链式训练；记录全程更新与best前缀更新，能力结论只在dev逐题审核后决定。

B Rule真实更新已证实：两步均非零LR且权重变化；best在第2步，保留了本阶段全部2次有效更新。继续既定同B阶段链，Loss不能证明规则能力改善。

B Strategy两次有效LR更新和504个Adapter张量变化已核验；最佳检查点仍为第二步，继续既定Tactics，不以Loss下降判定学习问题解决。

B三阶段合计6次非零LR更新已实测，所有best均第2步，final链没有丢失后续更新。B final接既定原23-dev控制输入而非skill投影输入，已逐条核验；推理复用原generate及原评分，单独原子输出目录。只能根据全部dev动作/语义证据作条件分支，当前不宣称能力改善。

B的7条规则dev未出现动作合法性改善，保留2局部混合、3无变化、2退步；尚不能提前决定完整B分支，继续其余16条原dev。语义维度沿用经SHA核验的A原审核，明确记录B新增非Classic角色和公开事实矛盾。

## 2026-09-16 B完整结果与C条件门禁

B未稳定改善且存在新的输出协议退化：format23→15、legal3→0、match1→0；counterfactual0/3→0/3，夜漏7/7。7混合、6不变、10退步全保留。unknown_type14→11是格式失败遮蔽，宽松未知词14不变，不进行别名归一或修正seat来救分。

有证据表明日程改变影响输出行为；暂不支持仅取消warmup改善核心能力。保持旧Schema、原dev、原评分、冻结数据和模型。按用户条件授权，在B完整报告commit/push后进入已冻结C_accum8；C从同Base新建Adapter，warmup=0与B一致，只把accum16改8。此实验同时改变批量/更新频率及对应cosine轨迹，不将其结果解释为纯步数因果；D、test、视频、Persona和v0.3不解锁。

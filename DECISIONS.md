...
## core离线诊断与单变量选择（2026-09-14）

- 完整v0.2结果已提交推送04608b1。本轮仅核查现有dev、训练曝光、日程和消息代码，未用test文本、新训练或视频。
- 确认Classic训练仅呈现当前角色技能，而旧评测携带14个通用字段。守卫输出读取无关checked_seats是可定位证据；不能据相关性认定全部失败由此引起。旧系统措辞、输入布局、动作词典也有差异，暂不同时改变。
- 选择v02_skill_projection_v0.1作为下一小实验，唯一变量为当前角色skill_state投影。冻结全部23条dev控制/处理输入及配置，保留旧系统提示/布局/事实/任务/权重/评分，复用旧控制回答。全部输入559～766/459～666 tokens，未截断。
- 先实现独立运行器及必要恢复测试，commit后生成；当前prepared_not_run。此dev敏感性实验不等于新数据集或正式验收；无论局部收益如何都不凭它解锁v0.3。
- 阶段训练角色明显不均：最后21条全为狼人，验证仅1条狼人发言；最佳由该阶段Loss选择。仍无证据确认跨阶段遗忘，故暂不同时增加epoch、混合回放或更换最佳选择标准。
- 三个诊断快照重复执行SHA/mtime不变，已有数据manifest与23条已审核预测SHA通过；报告清楚区分实测、静态检查和待验证假设。

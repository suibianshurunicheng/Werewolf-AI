...
OUTPUT = Path('reports/diagnostics/schedule_v0.1') / args.arm / 'dev'
PACKET = OUTPUT
...
def main():
    ...
    lines = ["# Training Schedule Diagnostic A vs " + args.arm + " 正式诊断报告", "", f"状态：{report['status']}；已比较{len(rows)}/23，语义审核{len(reviews)}/23。仅dev探索，非held-out能力验收。", "", report["conclusion"], "",
             "## 实验控制", "", "固定Base/revision、dataset_v0.2、Chat Template、LoRA/NF4及dev生成评分协议；唯一干预来自冻结训练日程矩阵。B仅取消warmup；C若执行则相对B仅accum16改8。A复用已完成v0.2，不重训。每组从同Base开始Rule再接本组Strategy/Tactics，按原验证Loss选择各阶段best/final。使用完全相同的原23条控制输入，包括未当前日程的skill_state；没有使用test调参，没有加入视频或Persona。", "",
    ...
    lines += ["", "未知词分布：" + json.dumps({s: report["summary"][s]["unknown_words"] for s in predictions}, ensure_ascii=False), "",
              "## 角色分组", "", "|角色/题数|合法 控制→当前日程|严格命中 控制→当前日程|语义变化|", "|---|---|---|---|"]
    ...
    lines += ["", "## 语义错误复核", "", "下表是模型语义审核，非独立专家盲审；仅统计两侧均可评估的维度，null不按正确计。动作拼写单独评分；语义技能、状态、权限错误依据原回答判定。", "", "|维度|配对可评估|控制错误|当前日程错误|", "|---|---:|---:|---:|"]
    ...
    lines += ["## 证据与限制", "", "原始回答、输入指纹和逐题SHA见本目录cases、run.json、paired_analysis.json及semantic_review.json。取消warmup使B每阶段LR从[0,5e-5]变为[5e-5,2.5e-5]，也改变总LR积分与Adam更新轨迹；A的零LR步仍积累动量。不能把结果归因于单独的更新次数，也不能证明整个失败已解释。只有单种子和反复诊断的23-dev，不支持held-out泛化结论。后续未执行实验明确记为尚未验证。", "",

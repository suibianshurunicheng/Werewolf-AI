...
- [ ] 核查完整v0.2训练覆盖；本批守卫、自救、角色边界在验证侧，须独立场景族补充，不能直接用该批训练。
- [x] strategy_batch_v0.1：24条/12对/10族，证据更新、团队动态调整、公开与私有分离；20训练侧/4验证侧，作者自审与完整81项测试通过。
- [ ] 汇总两批56条的能力覆盖表：补训练侧守卫/自救/角色边界/验人备选/票型关系，补独立战术验证族；保持既有场景族分组。
- [ ] 独立语义审核修正候选，保留作者自评与专家审核区别。
- [ ] 保存版本化Classic专用消息格式，裁剪扩展角色/技能/规则内容，不改v0.1消息。
- [ ] 冻结v0.2数据版本、训练配置、Tokenizer长度与dry-run结果，commit后训练。
...
- [ ] 在完整v0.2组装入口强制来源白名单和哈希审核，拒绝视频混入。
